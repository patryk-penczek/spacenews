      ...
      {Array.isArray(articlesMap) && articlesMap.length > 0 ? (
        articlesMap.map((result, index) => {
          return (
            <article
              key={index}
              className="grid h-32 w-full max-w-default grid-cols-12 items-center justify-center gap-x-2 p-4 font-light text-grayscale-500 dark:text-white sm:h-40 sm:gap-x-4 md:h-56 md:gap-x-8 lg:h-72 xl:h-96"
            >
              <a
                href={result.url}
                target="_blank"
